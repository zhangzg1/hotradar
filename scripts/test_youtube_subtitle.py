...
优先提取原生字幕（秒级），无字幕时回退到 Qwen ASR 转录（分钟级）

运行方式：
    conda activate ai-hotspot-monitor
    python scripts/test_youtube_subtitle.py                    # 使用默认URL测试
    python scripts/test_youtube_subtitle.py <URL>              # 测试指定URL
"""
import asyncio
import sys
import argparse
...
# 默认测试URL
DEFAULT_TEST_URLS = [
    ("https://www.youtube.com/watch?v=x2EllvOb2TY", "YouTube Shorts"),
]
...
def main():
    parser = argparse.ArgumentParser(description="测试 YouTube 视频音频转录")
