...
- **配额管理**：可配置各数据源的采集数量配额
- **Skill 模式**：支持作为 AI 编程助手（Claude Code、Codex 等）的 Skill 使用，无需部署完整服务即可进行热点采集
...
│   └── migrations/             # 数据库迁移
├── assets/                     # 静态资源
├── skills/                     # AI Agent Skill（轻量使用模式）
│   ├── README.md               # Skill 详细文档
│   ├── requirements.txt        # Skill Python 依赖
│   └── hotradar/               # HotRadar Skill
│       ├── SKILL.md            # Skill 定义
│       ├── scripts/            # 采集与推送脚本
│       └── references/         # AI 分析评分指南
...
如果你不需要完整的 Web 服务，只想在 AI 编程助手中快速追踪热点信息，可以使用 HotRadar Skill。它无需部署后端、数据库等基础设施，只需安装 Python 依赖即可运行。

#### 1. 安装 Skill

将项目中的 `skills/hotradar` 文件夹复制到对应工具的 skill 目录下：
...
source ai-hotspot-monitor/bin/activate
pip install -r skills/requirements.txt
```

#### 3. 使用 Skill

环境准备好后，在 AI 编程助手中用自然语言即可触发 HotRadar：
...
**在 Claude Code 中**，也可以输入 `/hotradar` 直接调用。

**在 Codex 中**，也可以输入 `$hotradar` 或 `/skills` 直接调用。

触发后，HotRadar 会逐步引导你配置关键词、数据源、抓取数量和邮件推送，确认后自动执行采集和 AI 分析。

#### 4. 采集结果与数据源

采集完成后会按关键词分组输出热点报告，包含标题、来源、相关性评分、重要性等级和摘要。支持 6 个数据源：Bing、搜狗、Bilibili（免费开箱即用），Twitter、YouTube、抖音（需提供凭据）。

> 安装步骤、使用方法、数据源详情等完整说明请参考 `skills/README.md`。
